...
from tvshow tv
inner join (
	select tv.idshow, min(if(ep.c12=0,9999,cast(ep.c12 as int))) as c12
	from episode ep
...
inner join (
	select tv.idshow, if(ep.c12=0,9999,cast(ep.c12 as int)) as c12, min(cast(ep.c13 as int)) as c13
	from episode ep
	inner join files fi on fi.idfile=ep.idfile
	inner join path pa on pa.idpath=fi.idpath
	inner join tvshow tv on tv.idshow=ep.idshow
...
inner join episode ep on ep.idshow=tv.idshow and ep.c12=if(nextseason.c12=9999,0,nextseason.c12) and ep.c13=nextepisode.c13
inner join files fi on fi.idfile=ep.idfile
inner join path pa on pa.idpath=fi.idpath
...
inner join (
	...
	select tv.idshow, 1 as priority, if(notplayedmindateadded.dateadded>maxlastplayed.lastplayed,notplayedmindateadded.dateadded,maxlastplayed.lastplayed) as updatetime
	from tvshow tv
	inner join (
