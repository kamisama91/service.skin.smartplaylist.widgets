select mo.idmovie as id, pa.strpath as path, fi.strfilename as file, ifnull(se.strset, '') as settitle, mo.c00 as title, ifnull(art.url, '') as poster, ifnull(arf.url, '') as fanart, fi.*
from movie mo
inner join (
	...
	inner join (
		select se.idset, (case when notplayedmindateadded.dateadded>maxlastplayed.lastplayed then notplayedmindateadded.dateadded else maxlastplayed.lastplayed end) as updatetime
		from sets se
		inner join (
	...
	where se.idset not in (
		...
		inner join path pa on pa.idpath=fi.idpath
		where #PLAYLIST_FILTER#
		and ((ifnull(fi.playcount,0) = 0 and fi.lastplayed is not null) 
		     or (ifnull(fi.playcount,0) > 0)))
